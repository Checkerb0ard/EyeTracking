...
namespace OscCore
{
    public sealed unsafe class OscServer : IDisposable
    {
        ...
        public OscServer(int port, int bufferSize = 4096)
        {
            ...
            {
                Core.Instance.LoggerInstance.Error($"port {port} is already in use, cannot start a new OSC Server on it");
                return;
            }
        ...
        void TryMatchPatterns(OscParser parser, byte* bufferPtr, int addressLength)
        {
            ...
            {
                AddressSpace.AddressToMethod.Add(stringBuffer, m_PatternMatchedMethods);
